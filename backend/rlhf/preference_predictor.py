...
class PreferencePredictor:
    ...
    def train_reward_model(self, sample):
        ...
        #entropy_loss = self._deprecated_compute_loss(sample)
        entropy_loss = self._compute_loss_batch(sample)
    ...
    def _deprecated_predicted_probability(self, trajectories):
        """
    ...
    def _deprecated_compute_loss(self, sample):
        """
        ...
        for trajectory_pair, human_feedback_label in sample:
            predicted_prob = self._deprecated_predicted_probability(trajectory_pair)
    ...
    def _compute_predicted_probability_batch(self, trajectories):
        ...
        rewards0 = self.reward_network(trajectory0.actions, trajectory0.states)
        rewards1 = self.reward_network(trajectory1.actions, trajectory1.states)
