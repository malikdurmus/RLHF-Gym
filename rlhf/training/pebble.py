...
from rlhf.training.surf import semi_supervised_labeling
        ...
        if global_step >= args.unsupervised_timesteps:
            ...
            if global_step % args.feedback_frequency == 0 or global_step == args.unsupervised_timesteps:
                ...
                # uniform-sampling
                    ...
                    trajectory_pairs = sampler.uniform_trajectory_pair(args.traj_length, args.feedback_frequency,args.synthetic_feedback)
                ...
                with preference_mutex:
                    if args.surf:       #TODO: we dont need the sampler class, everything in this class can be done in Replay buffers own methods
                        # ssl_preference_buffer consists of: augmented pairs and labels (generated by ssl)
                        ssl_preference_buffer = semi_supervised_labeling(preference_optimizer, sampler,args)

                        augmented_pb = preference_buffer.copy(apply_tda=True,min_length=args.min_crop_length, max_length=args.max_crop_length)
                        entropy_loss, ratio = preference_optimizer.train_reward_models_surf(augmented_pb, ssl_preference_buffer,
                                                                                       args.pref_batch_size)
                    else:
                        entropy_loss, ratio = preference_optimizer.train_reward_models(preference_buffer,
                                                                               args.pref_batch_size)
